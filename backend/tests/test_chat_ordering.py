...
def test_chat_history_orders_by_created_at_first():
    ...
    src = inspect.getsource(chat)
    assert "desc(ChatMessage.created_at)" in src, "Chat history must still order by created_at as the primary sort key"
...
def test_chat_history_has_both_order_columns():
    ...
    assert any("created_at" in l and "id" in l for l in order_lines), (
