...
def test_no_print_calls_in_db_module():
    ...
    lines = src.split("\n")
    print_lines = [l.strip() for l in lines if l.strip().startswith("print(") and not l.strip().startswith("#")]
    assert not print_lines, f"db.py must use logger.info() not print(). Found: {print_lines}"
...
def test_logger_used_in_db_module():
    ...
    assert "logger.info" in src or "logger.warning" in src, "db.py must call logger.info() or logger.warning()"
