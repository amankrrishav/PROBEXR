...
def test_auth_router_instruments_login_failure():
    ...
    assert 'AUTH_EVENTS_TOTAL.labels(event="login_failure")' in src, "login endpoint must record login_failure metric"
...
def test_auth_router_instruments_login_success():
    ...
    src = inspect.getsource(auth_router.login)
    assert 'AUTH_EVENTS_TOTAL.labels(event="login_success")' in src, "login endpoint must record login_success metric"
...
def test_auth_router_instruments_logout():
    ...
    assert 'AUTH_EVENTS_TOTAL.labels(event="logout")' in src, "logout endpoint must record logout metric"
...
def test_auth_router_instruments_token_refresh():
    ...
    assert 'AUTH_EVENTS_TOTAL.labels(event="token_refresh")' in src, "refresh endpoint must record token_refresh metric"
...
def test_logging_middleware_records_http_metrics():
    ...
    assert "HTTP_REQUEST_DURATION_SECONDS" in src, "LoggingMiddleware must record request duration metric"
    assert "HTTP_REQUESTS_TOTAL" in src, "LoggingMiddleware must record request count metric"
