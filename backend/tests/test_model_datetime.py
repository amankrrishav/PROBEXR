...
def test_chat_model_no_utcnow():
    ...
    assert "utcnow()" not in src, "app/models/chat.py must not use datetime.utcnow() — deprecated in Python 3.12"


def test_flashcards_model_no_utcnow():
    src = _get_model_source("app.models.flashcards")
    assert "utcnow()" not in src, "app/models/flashcards.py must not use datetime.utcnow()"


def test_synthesis_model_no_utcnow():
    src = _get_model_source("app.models.synthesis")
    assert "utcnow()" not in src, "app/models/synthesis.py must not use datetime.utcnow()"


def test_tts_model_no_utcnow():
    src = _get_model_source("app.models.tts")
    assert "utcnow()" not in src, "app/models/tts.py must not use datetime.utcnow()"
...
def test_user_created_at_has_default_factory():
    ...
    assert any(
        "default_factory" in l or "default_factory" in "\n".join(src.split("\n")[i : i + 3])
