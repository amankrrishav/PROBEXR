...
class TestTextRankSentenceLimit:
    def test_sentence_cap_constant_defined(self):
        ...
        assert "_SENTENCE_LIMIT" in src, "extractive.py must define _SENTENCE_LIMIT for the O(N²) guard"
    ...
    def test_long_text_completes_in_reasonable_time(self):
        ...
        assert "summary" in result
        assert elapsed < 10.0, f"TextRank took {elapsed:.1f}s on 400-sentence text — sentence cap may not be firing"
    ...
    def test_long_text_still_produces_summary(self):
        ...
        text = _make_long_text(400)
        result = summarize_extractive(text, min_words=30)
        assert len(result["summary"].split()) >= 10, "Summary should have at least 10 words even after sentence cap"
