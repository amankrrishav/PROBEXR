...
@pytest.mark.asyncio
async def test_create_flashcards_no_llm_provider_returns_400(authed_client: AsyncClient, document_id: int):
    """
...
@pytest.mark.asyncio
async def test_create_flashcards_no_llm_error_message_is_actionable(authed_client: AsyncClient, document_id: int):
...
def test_list_flashcard_sets_no_loop_query():
    ...
    assert "set_ids" in src, "list_flashcard_sets must batch card count lookup using set_ids"
    assert ".in_(set_ids)" in src, "list_flashcard_sets must use .in_(set_ids) to batch the card count query"
...
def test_flashcard_front_has_max_length():
    ...
    assert any("max_length" in l for l in front_lines), f"Flashcard.front must have max_length. Found: {front_lines}"
