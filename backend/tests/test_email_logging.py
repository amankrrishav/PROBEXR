...
def test_no_print_calls_in_email_service():
    ...
    assert not lines, f"email.py must use logger.debug() not print(). Found: {lines}"


def test_dev_fallback_uses_logger_debug():
    """Dev fallback (no SMTP) must log via logger.debug(), not print()."""
    src = _src()
    assert "logger.debug" in src, "email.py dev fallback must use logger.debug() to route through the logging pipeline"
...
def test_no_fstring_logger_calls():
    ...
    assert not fstring_logger, f"email.py must use %-style logger formatting, not f-strings. Found: {fstring_logger}"
